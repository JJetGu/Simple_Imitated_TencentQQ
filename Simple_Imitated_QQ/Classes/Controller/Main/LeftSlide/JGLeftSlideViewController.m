...
@interface JGLeftSlideViewController ()<UIGestureRecognizerDelegate>
{
...
@implementation JGLeftSlideViewController
...
-(instancetype)initWithLeftView:(UIViewController *)leftVC andMainView:(UIViewController *)mainVC
{
    if (self = [super init]) {
        ...
        cover.frame = self.leftVC.view.bounds;
        cover.backgroundColor = [UIColor blackColor];
        cover.alpha = 0.5;
        self.contentView = cover;////////////////////???????
        [self.leftVC.view addSubview:cover];
...
#pragma mark - 行为收敛控制 -- 在主视图（右视图）添加点击手势
